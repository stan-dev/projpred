...
#' @export
project_poc <- function(object, nv = NULL, vind = NULL, relax = NULL, ns = NULL, nc = NULL,
  ...
  if (is.null(nv)) {
	...
	if (relax) {
	  ...
	  subm <- .get_submodels_poc(object$spath, nv, family_kl, p_ref,
                               refmodel, intercept, regul, as.search=T)
	}

	# add family_kl
	proj <- lapply(subm, function(model) {
	  names(model$vind) <- sapply(model$vind, function(i, ch) names(ch)[which(ch == i)][1],
	                          object$vind)
