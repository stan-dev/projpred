...
#' transformation.
linear_multilevel_mle <- function(formula, data, weights = NULL, offset = NULL,
                                  regul=NULL) {
  formula <- validate_response_formula(formula)
  fit_lmer_callback <- function(f) {
    tryCatch(lme4::lmer(f, data = data, weigts = weights, offset = offset),
             error=function(e) {
               if (grepl("No random effects", as.character(e)))
                 lm(f, data = data, weights = weights, offset = offset)
               else if (grepl("not positive definite", as.character(e)))
                 lme4::lmer(f, data = data, weights = weights, offset = offset,
