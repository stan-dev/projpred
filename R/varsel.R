...
#' If not provided, training data is used. Currently this argument is for
#' internal use only.
...
#' determined by \code{regul}). If FALSE, then the coefficients are the solution
#' from the' L1-penalized projection. This option is relevant only if
#' \code{method}='L1'. Default is TRUE for genuine reference models and FALSE if
#' \code{object} is datafit (see \link[=init_refmodel]{init_refmodel}).
...
#'    continued. Defaults to min(20, D, floor(0.4*n)) where n is the number
#'    of observations and D the number of variables.
...
#' variables. Zero means that those variables have no cost and will therefore be
#' selected first, whereas Inf means those variables will never be selected.
#' Currently works only if method == 'L1'. By default 1 for each variable.
...
#' need for regularization, but sometimes for some models the projection can be
...
varsel <- function(object, d_test = NULL, method = NULL, number_samples = NULL,
                   ...
                   regul = 1e-4, penalty = NULL, groups = NULL, ...) {
  ...
  args <- parse_args_varsel(
    ...
    number_clusters_pred, number_samples_pred, groups
  )
  method <- args$method
  ...
  p_sub <- .get_submodels(search_path, c(0, seq_along(vind)), family, p_pred,
...
select <- function(method, p_sel, refmodel, family, intercept, nv_max,
  ...
  if (method == "l1") {
    ...
    tryCatch(search_path <- search_forward(p_sel, refmodel, family,
      intercept, nv_max, verbose,
      opt,
      groups = groups
    ),
    "error" = .varsel_errors
...
parse_args_varsel <- function(refmodel, method, cv_search, intercept, nv_max,
  ...
  ##
  if (is.null(groups)) {
    groups <- split_formula(refmodel$formula)
  }
  has_group_features <- formula_contains_group_terms(refmodel$formula)
  ...
  nlist(method, cv_search, intercept, nv_max, number_clusters, number_samples,
        number_clusters_pred, number_samples_pred, groups)
}
