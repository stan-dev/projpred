...
The latent projection (Catalina et al., 2021) is a quite general principle for extending **projpred**'s traditional projection to more response families. The families for which full latent-projection functionality (in particular, post-processing on the original response scale) is currently available are `binomial()`^[Currently, the support for the `binomial()` family does not include binomial distributions with more than one trial. In such a case, a workaround is to de-aggregate the Bernoulli trials which belong to the same (aggregated) observation, i.e., to use a "long" dataset.] (again also `brms::bernoulli()`), `brms::cumulative()`, and `rstanarm::stan_polr()` fits. The latent projection is applied when setting argument `latent` of `extend_family()` (which is called by `init_refmodel()`) to `TRUE`. Note that the latent projection is currently considered as an experimental feature since it is subject to some more theoretical investigations.
