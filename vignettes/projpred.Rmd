...
Note that we have cut off the search at `nterms_max = 10` (which is smaller than the number of predictor terms in the full model, 20 here), so the ranking proportions in the matrix returned by `props()` do not need to sum to 100 % (neither column-wise nor row-wise).
