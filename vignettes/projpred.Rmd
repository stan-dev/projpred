...
Based on that plot, we would decide for a submodel size of 8 because that's the point where the performance measures level off and are close enough to the reference model's performance.
```{r}
modsize_decided <- 8
```

Note that **projpred** offers the `suggest_size()` function which may help in the decision for a submodel size, but this is a rather heuristic method and needs to be interpreted with caution (see the `?suggest_size` help).
```{r}
suggest_size(cvvs)
```
Here, we would get the same final submodel size (`8`) as by our manual decision.
...
Combining the decided submodel size of 8 with the solution path leads to the following terms (as well as the intercept) as the predictor terms of the final submodel:
```{r}
