...
The predictive performance statistic(s) plotted by `plot.vsel()` can be retrieved in tabular form via `summary.vsel()`.
...
For example, we can create the summary table matching the predictive performance plot above (and also adjust the minimum number of printed significant digits):
```{r smmry_vsel}
