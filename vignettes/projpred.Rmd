...
Here, we decide for a submodel size of 7 because it seems to provide the best trade-off between sparsity and accuracy (size 7 is the smallest size where the submodel MLPD is close enough to the reference model MLPD).
```{r size_man}
size_decided <- 7
```
...
```
With this heuristic, we would get the same final submodel size (`7`) as with our manual decision.
