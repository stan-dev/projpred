...
What is special about the projection predictive variable selection is that it not only performs a variable selection, but also allows for valid post-selection inference.
...
In **projpred**, the projection predictive variable selection consists of a *search* part and an *evaluation* part.
...
There are two functions for performing the variable selection: `varsel()` and `cv_varsel()`.
In contrast to `varsel()`, `cv_varsel()` performs a cross-validation (CV) by running the search part with the training data of each CV fold separately (an exception is `validate_search = FALSE`, see `?cv_varsel` and below) and running the evaluation part on the corresponding test set of each CV fold.
Because of this CV, `cv_varsel()` with `validate_search = TRUE` is recommended over `varsel()` and `cv_varsel()` with `validate_search = FALSE`.
Thus, we use `cv_varsel()` with `validate_search = TRUE` here.
Nonetheless, running `varsel()` or `cv_varsel()` with `validate_search = FALSE` first can give a rough idea of the performance of the submodels.
A more principled **projpred** workflow is work under progress.
...
Here, we use only some of the available arguments; see the documentation of `cv_varsel()` for the full list of arguments.
By default, `cv_varsel()` runs a Pareto-smoothed importance sampling [PSIS, see @vehtari_practical_2017; @vehtari_pareto_2022] leave-one-out (LOO) CV (see argument `cv_method`) and includes the search in that CV (see argument `validate_search`).
Here, we use a $K$-fold CV (which currently always includes the search in the CV, i.e., always comes with `validate_search = TRUE`) and `K = 2` to make this vignette build faster.
For final results in practice, we recommend using either the default of `cv_method = "LOO"` (with `validate_search = TRUE`) or a larger value for `K` if this is possible in terms of computation time.
Here, we also set `nclusters_pred` to a low value of `20` to speed up the building of the vignette.
By modifying argument `nterms_max`, we impose a limit on the submodel size up to which the search is continued.
Typically, one has to run the variable selection with a large `nterms_max` first (the default value may not even be large enough) and only after inspecting the results from this first run, one is able to set a reasonable `nterms_max` in subsequent runs.
The value we are using here (`9`) is based on such a first run (which is not shown here, though).
```{r cv_varsel, results='hide', message=FALSE}
cvvs <- cv_varsel(
  refm_fit,
  cv_method = "kfold",
  ### Only for the sake of speed (not recommended in general):
...
To decide for a submodel size, there are several performance statistics we can plot as a function of the submodel size.
Here, we use the mean log predictive density (MLPD; see argument `stats` of `summary.vsel()` for details) and the root mean squared error (RMSE).
...
Since the differences are usually of more interest (with regard to the submodel size decision), we directly plot with `deltas = TRUE` here:
```{r plot_vsel, fig.asp=1.5 * 0.618}
plot(cvvs, stats = c("mlpd", "rmse"), deltas = TRUE, seed = 54548)
```

Based on that plot (see `?plot.vsel` for a description), we would decide for a submodel size of 6 because that's the point where the performance measures level off and are close enough to the reference model's performance:
```{r size_man}
modsize_decided <- 6
```
