...
Here, we choose the mean log predictive density (MLPD; see argument `stats` of `summary.vsel()` for details) as the only performance statistic:
```{r plot_vsel_fast}
...
Based on that plot, we decide for a submodel size of 6 because that's the point where the submodel MLPD levels off and is close enough to the reference model MLPD:
```{r size_man}
modsize_decided <- 6
```
...
```
With this heuristic, we would get the same final submodel size (`6`) as by our manual decision.
