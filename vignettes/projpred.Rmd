...
If `validate_search = FALSE` (which is currently only available for `cv_method = "LOO"`), then the search is excluded from the CV so that only a single full-data search is run.
...
Now that we have made a decision for the final submodel size, we inspect further results from the `cv_varsel()` run and, in particular, the predictor ranking.
