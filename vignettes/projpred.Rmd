...
The search part determines the solution path, i.e., the best submodel for each submodel size (the size is the number of predictor terms).
The evaluation part determines the predictive performance of the submodels along the solution path.
...
<!-- then the CV only includes the evaluation part, which means that there is only a single full-data search and that the projection onto the submodels along the resulting full-data solution path takes place with the training data of each CV fold separately (the actual performance evaluation itself is then achieved as usual by making predictions for the test data of each CV fold, now based on the submodels along the full-data solution path for which the projection was performed with the training data corresponding to that fold, and comparing these predictions to the truth, with the nature of the comparison depending on the chosen predictive performance statistic). -->
...
To illustrate a preliminary `cv_varsel()` run with `validate_search = FALSE`, we try `nterms_max = 10`, i.e., half the number of predictor terms in the full model:
...
To find a suitable value for `nterms_max` in subsequent runs, we take a look at a plot of predictive performance statistics in dependence of the submodel size.
As performance statistics, we use the mean log predictive density (MLPD; see argument `stats` of `summary.vsel()` for details) and the root mean squared error (RMSE):
```{r plot_vsel_fast, fig.asp=1.5 * 0.618}
plot(cvvs_fast, stats = c("mlpd", "rmse"), seed = 54548)
```
This plot (see `?plot.vsel` for a description) shows that the predictive performance of the submodels does not change much after submodel size 8, so we set `nterms_max = 8` in our final `cv_varsel()` run^[Here, we were lucky that half the number of predictor terms in the full model (`nterms_max = 10`) was a sufficiently large starting point. In practice, half the number of predictor terms in the full model may not be large enough to have submodel performances that level off before this point.].
...
We also set `nclusters_pred` to a low value of `20` to speed up the building of the vignette:
...
<!-- This should be the first step (in particular, before inspecting the solution path) in order to avoid a user-induced selection bias (which could occur if the user made the submodel size decision dependent on the solution path). -->
...
```{r plot_vsel, fig.asp=1.5 * 0.618}
plot(cvvs, stats = c("mlpd", "rmse"), deltas = TRUE, seed = 54548)
```

Based on that plot, we decide for a submodel size of 6 because that's the point where the performance measures level off and are close enough to the reference model's performance:
```{r size_man}
modsize_decided <- 6
```
...
```
With both performance statistics, we would get the same final submodel size (`6`) as by our manual decision.

Now that
<!-- Only now, after -->
we have made a decision for the final submodel size, we inspect further results from the `cv_varsel()` run and, in particular, the solution path.
...
By the following code, `project()` will project the reference model onto the final submodel once again^[During the forward search, the reference model has already been projected onto all candidate models (this was where arguments `ndraws` and `nclusters` of `cv_varsel()` came into play). For the evaluation of the submodels along the solution path, the reference model has been projected onto these submodels again (this was where arguments `ndraws_pred` and `nclusters_pred` of `cv_varsel()` came into play). In principle, one could use the re-projections from the evaluation part for post-selection inference, but due to a deficiency in the current implementation (see GitHub issue #168), we currently have to project once again.]:
```{r project}
...
Sometimes, the ordering of the predictor terms in the solution path makes sense, but for an increasing submodel size, the performance measures of the submodels do not approach that of the reference model so that the submodels exhibit a predictive performance that stays worse than the reference model's.
