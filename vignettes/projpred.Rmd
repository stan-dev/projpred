...
Here, we will use the **rstanarm** package to fit the reference model. If you want to use the **brms** package, you simply have to replace the **rstanarm** fit (of class `stanreg`) in all the code below by your **brms** fit (of class `brmsfit`).
```{r}
library(rstanarm)
```
...
```
This plot shows that it is already the reference model which performs badly in its PPC. Thus, we would now have to re-iterate and improve the reference model, see Gelman et al. (2020). That would go beyond the scope of this vignette, however, so we'll stop here.

## Troubleshooting

Sometimes, the ordering of the predictor terms in the solution path makes sense, but the performance measures of the projections do not approach that of the reference model for increasing model size. There may be different reasons that can explain this behavior:
