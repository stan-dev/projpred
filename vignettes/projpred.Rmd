...
Only note that in case of a **brms** fit, we recommend to specify argument `brms_seed` of `brms::get_refmodel.brmsfit()`.
...
The evaluation part determines the predictive performance of the submodels along the predictor ranking.
...
If `validate_search = FALSE` (which is currently only available for `cv_method = "LOO"`),
then the search is excluded from the CV so that only a single full-data search is run.
<!-- then the CV only includes the evaluation part, which means that there is only a single full-data search and that the projection onto the submodels along the resulting full-data predictor ranking takes place with the training data of each CV fold separately (the actual performance evaluation itself is then achieved as usual by making predictions for the test data of each CV fold, now based on the submodels along the full-data predictor ranking for which the projection was performed with the training data corresponding to that fold, and comparing these predictions to the truth, with the nature of the comparison depending on the chosen predictive performance statistic). -->
Because of its most thorough protection against overfitting^[Note that currently, neither `varsel()` nor `cv_varsel()` (not even `cv_varsel()` with `validate_search = TRUE`) guard against overfitting in the selection of a submodel *size*. This is why we added "approximately" to "valid post-selection inference" in section ["Introduction"](#intro). Typically, however, the overfitting induced by the size selection should be comparably small.], `cv_varsel()` with `validate_search = TRUE` is recommended over `varsel()` and `cv_varsel()` with `validate_search = FALSE`.
Nonetheless, a preliminary and comparably fast run of `varsel()` or `cv_varsel()` with `validate_search = FALSE` can give a rough idea of the performance of the submodels and can be used for finding a suitable value for argument `nterms_max` in subsequent runs (argument `nterms_max` imposes a limit on the submodel size up to which the search is continued and is thus able to reduce the runtime considerably).
...
<!-- (in practice, in the absence of a more informed guess, the default value for `nterms_max` may be another good starting point, but it may not be large enough). -->
...
To find a suitable value for `nterms_max` in subsequent runs, we take a look at a plot of at least one predictive performance statistic in dependence of the submodel size.
...
```{r plot_vsel_fast}
plot(cvvs_fast, stats = "mlpd")
```
This plot (see `?plot.vsel` for a description) shows that the submodel MLPD does not change much after submodel size 8, so in our final `cv_varsel()` run, we set `nterms_max` to a value slightly higher than 8 (here: `nterms_max = 10`) to ensure that we see the MLPD leveling off.
...
We can now select a final submodel size.
<!-- The first step after running `varsel()` or `cv_varsel()` should be the decision for a final submodel size. -->
<!-- This should be the first step (in particular, before inspecting the predictor ranking) in order to avoid a user-induced selection bias (which could occur if the user made the submodel size decision dependent on the predictor ranking). -->
By default, the performance statistics are plotted on their original scale, but with `deltas = TRUE`, they are calculated as differences from a baseline model (which is the reference model by default, at least in the most common cases).
Since the differences are usually of more interest (with regard to the decision for a final submodel size), we directly plot with `deltas = TRUE` here:
```{r plot_vsel}
...
Sometimes (as here), the plot may be ambiguous and one may have to find a suitable trade-off between predictive performance (accuracy) and model size (sparsity) in the context of subject-matter knowledge.
...
Now that
<!-- Only now, after -->
we have made a decision for the final submodel size, we inspect further results from the `cv_varsel()` run and, in particular, the predictor ranking.
For example, we could simply `print()` the resulting `cvvs` object, but to create the summary table matching the predictive performance plot above (and to adjust the minimum number of printed significant digits), we slightly adapt the printing here:
```{r smmry_vsel}
print(summary(cvvs, stats = "mlpd", deltas = TRUE), digits = 1)
```
The predictor ranking from the `print()` output (column `solution_terms`) is also accessible through the `solution_terms()` function:
```{r soltrms}
( soltrms <- solution_terms(cvvs) )
```

When retrieved via `solution_terms()`, the predictor ranking is always based on the full-data search.
...
In case of `cv_varsel()` with `validate_search = TRUE` (like here), it usually makes sense to investigate the ranking proportions derived from the fold-wise predictor rankings, for example to get a sense for the variability in the ranking of the predictors.
...
To compute these ranking proportions, we use `props()`:
```{r props}
( pr_rk <- props(rk) )
```
Here, the ranking proportions are of little use as we have set `K = 2` (in the final `cv_varsel()` call above) for the sake of speed.
Nevertheless, we can see that the two CV folds agree on the two most relevant predictor terms `X1` and `X14` as well as on their order.
From object `soltrms` above, we can see that `X1` and `X14` are also the two most relevant predictor terms in the full-data predictor ranking.

Note that we have cut off the search at `nterms_max = 10`, so the ranking proportions in the matrix returned by `props()` do not need to sum to 100 % (neither column-wise nor row-wise).

The matrix of ranking proportions can also be visualized via `plot.props()`:
```{r plot_props, fig.width = 6, out.width = "80%"}
plot(pr_rk)
```

Combining the decided submodel size of 6 with the full-data predictor ranking leads to the following terms (as well as the intercept) as the predictor terms of the final submodel:
```{r soltrms_final}
...
By the following code, `project()` will project the reference model onto the final submodel once again^[During the forward search, the reference model has already been projected onto all candidate models (this was where arguments `ndraws` and `nclusters` of `cv_varsel()` came into play). For the evaluation of the submodels along the predictor ranking, the reference model has been projected onto these submodels again (this was where arguments `ndraws_pred` and `nclusters_pred` of `cv_varsel()` came into play). In principle, one could use the re-projections from the evaluation part for post-selection inference, but due to a deficiency in the current implementation (see GitHub issue #168), we currently have to project once again.]:
```{r project}
...
<!-- ) -->
<!-- ``` -->
For more accurate results, we could have increased argument `ndraws` of `project()` (up to the number of posterior draws in the reference model).
However, this increases the runtime, which we don't want in this vignette.
