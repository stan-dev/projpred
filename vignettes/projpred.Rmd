...
As highlighted by the message printed above, the predictor ranking from column `solution_terms` is based on the full-data search.
...
The predictor terms `X1` and `X14` are also the two most relevant ones in the full-data predictor ranking:
```{r ranking_fulldata}
rk$fulldata
```
...
```{r predictors_final}
( predictors_final <- head(rk$fulldata, modsize_decided) )
```
