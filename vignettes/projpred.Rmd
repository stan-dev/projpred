...
By default, the performance statistics are plotted on their original scale and the uncertainty bars match this scale, but argument `deltas` offers two more options:
...
*   With `deltas = "mixed"`, the performance statistics (i.e., their point estimates) are plotted on the original scale, but the uncertainty bars visualize the difference-scale uncertainty.

Since the difference-scale uncertainty is usually of more interest than the original-scale uncertainty (at least with regard to the decision for a final submodel size) and the original-scale point estimates are often of more interest than the difference-scale point estimates, we plot with `deltas = "mixed"` here:
```{r plot_vsel}
