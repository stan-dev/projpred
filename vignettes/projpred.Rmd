...
Since the column names of the matrix returned by `props()` follow the full-data predictor ranking, we can infer that `X1` and `X14` are also the two most relevant predictor terms in the full-data predictor ranking.
To see this more explicitly, we can access element `fulldata` of the `ranking()` output:
```{r ranking_fulldata}
rk[["fulldata"]]
```

Note that we have cut off the search at `nterms_max = 10` (which is smaller than the number of predictor terms in the full model, 20 here), so the ranking proportions in the `pr_rk` matrix do not need to sum to 100 % (neither column-wise nor row-wise).
...
```{r plot_props_cumul, fig.width = 6, out.width = "80%"}
plot(props(rk, cumulate = TRUE))
```
This plot tells us that both CV folds have all terms from `predictors_final` (except for `X6`) included among the first 6 terms of their predictor ranking (6 was the chosen submodel size, so we look at `<=6` on the x-axis).
Only one of the two CV folds includes `X6` among the first 6 terms, but the other CV fold includes it shortly after that (at size 7, see also the `plot(pr_rk)` call above), which indicates that `X6` might indeed be relevant.
...
From the plot, we can see that size 8 might be an unfortunate choice because cutting off the full-data predictor ranking at that size would have led to the selection of `X11` which is not included among the first 8 terms by any CV fold.
Size 7 would probably be a better choice than size 8 because cutting off the full-data predictor ranking at size 7 would have led to the selection of `X8` which is included among the first 7 terms by one CV fold and among the first 8 terms by the other CV fold (see also the `plot(pr_rk)` call above).
However, since `K = 2` is too small for reliable statements regarding the variability in the ranking of the predictors, we did not take the cumulated ranking proportions into account when we made our decision for a submodel size above.
...
By the following code, `project()` will project the reference model onto the final submodel once again^[During the forward search, the reference model has already been projected onto all candidate models (this was where arguments `ndraws` and `nclusters` of `cv_varsel()` came into play). For the evaluation of the submodels along a predictor ranking (either full-data or fold-wise), the reference model has been projected onto these submodels again (this was where arguments `ndraws_pred` and `nclusters_pred` of `cv_varsel()` came into play). In principle, one could use the re-projections from the evaluation part for post-selection inference, but due to a deficiency in the current implementation (see GitHub issue #168), we currently have to project once again.]:
```{r project}
