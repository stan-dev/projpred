...
Since the difference-scale uncertainty is usually more helpful than the actual-scale uncertainty (at least with regard to the decision for a final submodel size) and the actual-scale point estimates are often more helpful than the difference-scale point estimates, we plot with `deltas = "mixed"` here (and we also set `show_cv_proportions = TRUE`---via its global option---for illustrative purposes, which becomes clearer down below):
```{r plot_vsel}
