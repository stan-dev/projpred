...
In principle, the projection predictive variable selection requires only little information about the form of the reference model. For a "typical" reference model as shown above, all candidate models are actual *sub*models of the reference model and thus, many aspects of the reference model coincide with those from the submodels. However, this does not need to be the case: For "custom" reference models, the candidate models don't need to be actual *sub*models of the reference model (although they will still be actual *sub*models of the full `formula` used by the search procedure---which does not have to be the same as the reference model's `formula`, if the reference model possesses a `formula` at all). This explains why the following remarks from this section refer to the submodels and not to the reference model.
