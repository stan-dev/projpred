...
```{r, cache=TRUE, fig.width=6, fig.height=2}
...
We make predictions with the projected submodels. For point estimates we can use the method `proj_linpred`. Test inputs can be provided using the keyword `newdata`. If also the test targets `ynew` are provided, then the function evaluates the log predictive density at these points. For instance, the following computes the mean of the predictive distribution and evaluates the log density at the training points using the 2 most relevant variables.
```{r, cache=TRUE}
...
```{r, fig.width=5, fig.height=3}
ggplot() +
  geom_point(aes(x = pred$pred, y = data_pois$phen_pois)) +
  geom_abline(slope = 1, color = "red") +
  labs(x = "prediction", y = "y")
```
...
subsample 50 individuals (for which sampling still takes a bit). For this, we use the great `tidyverse` environment.
...
pred <- proj_linpred(cv_vs_va, newdata = VerbAgg_subsample,
                     ...
                     nterms = 3, integrated = TRUE, ndraws = 10)
```

Visualize the predictions
```{r, fig.width=5, fig.height=3}
ggplot() +
  geom_point(aes(x = pred$pred, y = VerbAgg_subsample$r2num)) +
  geom_abline(slope = 1, color = "red") +
  labs(x = "prediction", y = "y")
```
