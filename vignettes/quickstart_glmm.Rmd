...
most relevant terms `(1 | Subject)` and `age`. For the multilevel term
`(1 | Subject)`, we visualize its (hierarchical) standard deviation.
...
projected draws. These draws can be visualized with, for example, the
`mcmc_areas` function in the `bayesplot` package. Below, we'll do so for the two
most relevant terms `cofactor` and `(1 | obs)`. For the multilevel term
`(1 | obs)`, we visualize its (hierarchical) standard deviation.
