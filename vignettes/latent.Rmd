...
As illustrated by the Poisson example below, the latent projection can not only be used for non-exponential-family response distributions, but it can also be beneficial for exponential-family response distributions.
...
In this example, we will illustrate that in case of an(**TODO (latent)**: "a multilevel model with an"?) exponential-family response distribution (here the Poisson distribution), the latent projection can improve runtime and lead to a more stable variable selection compared to **projpred**'s traditional projection.
...
# the test dataset):
N <- 100
sim_poiss <- function(nobs = 2 * N, ncon = 10, ngrpPL = 4, ngrpGL = 1,
                      nnoise = 39) {
  ...
  ### TODO: Make `grpGL` have a larger effect (and perhaps also increase
  ### `ngrpGL` to see lme4 issue #682 more clearly):
...
### TODO: Remove this as soon as a repair_re.glmmPQL() method has been added:
stopifnot(identical(levels(dat_poiss_train$grpGL),
                    levels(dat_poiss_test$grpGL)))
### 
```
...
### TODO (latent): Use the appropriate sigma_tilde from piironen_sparsity_2017:
...
refm_fml <- as.formula(paste("y", "~", paste(
  c(grep("^x|^grpPL", names(dat_poiss_train), value = TRUE)), # "(1 | grpGL)", 
  collapse = " + "
)))
refm_fit_poiss <- stan_glm( #er
  formula = refm_fml,
  family = poisson(),
...
We now run a variable selection with `latent = TRUE` to request the latent projection (since we have a hold-out test dataset available, we can use `varsel()` with argument `d_test` instead of `cv_varsel()`). We measure the runtime to compare it to the traditional projection's one later:
```{r}
library(projpred)
```
...
```
On absolute scale (column `mlpd`), we see that submodel size 11 leads to an MLPD of `-2.694`, i.e., a geometric mean predictive probability (GMPP) of `exp(-2.694)` which is ca. `r paste(100 * round(exp(-2.694), 3), "%")` whereas size 12 leads to a GMPP of ca. `r paste(100 * round(exp(-2.238), 3), "%")`. This is a considerable improvement from size 11 to size 12, so another justification for size 12. (Size 13 would have resulted in a GMPP of ca. `r paste(100 * round(exp(-2.221), 3), "%")` which is not so big of an improvement compared to size 12.)

In the solution path up to the selected size of 12, we can see that one noise term (`xn.16`) has slipped among the selected predictor terms. We can see this more explicitly using the following code:
```{r}
soltrms_lat <- solution_terms(cvvs_lat)
( soltrms_lat_final <- head(soltrms_lat, modsize_decided_lat) )
```
This suboptimal solution path is due to the L1 search. A forward search would have given the oracle solution path, i.e., first the truly relevant predictors and only then the noise predictors. However, for speeding up the building of this vignette, we use L1 search here.
...
```{r, results='hide'}
# Activate option `projpred.PQL` to use MASS::glmmPQL() as GLMM submodel fitter:
...
# modsize_decided_trad <- 
suggest_size(cvvs_trad, stat = "mlpd")
smmry_trad <- summary(
  cvvs_trad,
  stats = "mlpd",
  type = c("mean", "se", "lower", "upper", "diff", "diff.se")
...
```
Again, a forward search would have resulted in a slightly better behavior of the variable selection (in particular, for very large model sizes, the model size selection curve would eventually have approached the reference model line), but the selection would still have been unstable and some noise terms would still have been selected too early. So, for the sake of speed, we have chosen L1 search here in this vignette.

```{r}
# stopifnot(inherits(cvvs_trad$search_path$submodls[[8]][[1]], "glmmPQL"))
```
