...
```{r}
### Comment by FW: I haven't checked whether the code in
### generate_fake_poisson_data() actually makes sense.
...
```{r, message=FALSE, warning=FALSE}
### Comment by FW: I haven't checked whether the model actually makes sense.
...
```{r}
### Currently, extract_eta() is not necessary. (But perhaps in the future? The
### body of extract_eta() hints at plans for extending this).
...
```{r, message=FALSE, warning=FALSE}
### Comment by FW: I haven't checked whether the model actually makes sense. The
### brm() call below fails on my machine. Perhaps `1 - status` is incorrect?
