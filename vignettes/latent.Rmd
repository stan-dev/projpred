...
In conclusion, this example showed that the latent projection can be advantageous also for exponential-family models by improving the runtime and providing a more stable submodel size selection plot. The latter holds especially in case of L1 search, but also (even if not that strongly) for forward search. For the former (the runtime), it's the other way around.
