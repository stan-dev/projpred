...
To this end, the latent predictor is assumed to have a Gaussian distribution, since it (i) constitutes a combination of predictor data and regression parameters which is often linear (in the parameters, but---less---often also in the predictor data) or at least additive (across the predictor terms) and (ii) has the complete real line as support. Furthermore, the Gaussian distribution has the highest differential entropy over all distributions with two finite moments and with the real line as support [see, e.g., @cover_elements_1991]. In some cases, e.g., for the probit link, the Gaussian distribution is even part of the original statistical model. In case of the logit link, the Gaussian distribution with a standard deviation of 1.6 approximates the logistic distribution (with a scale parameter of 1).
...
```{r plot_vsel_lat, fig.width = 6, out.width = "75%"}
...
```{r plot_vsel_lat_zoom, fig.width = 6, out.width = "75%"}
...
We can also inspect further results from the `varsel()` run and, in particular, the predictor ranking (also known as solution path):
```{r smmry_vsel_lat}
smmry_lat <- summary(
  vs_lat,
  stats = "mlpd",
...
```
On absolute scale (column `mlpd`), we see that submodel size `r if (!params$EVAL) character() else size_decided_lat - 1` leads to an MLPD of ``r if (!params$EVAL) character() else round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == size_decided_lat - 1], 2)``, i.e., a geometric mean predictive density (GMPD; due to the discrete response family, the "density" values are probabilities here, so we will report the GMPD as percentage) of ``r if (!params$EVAL) character() else paste0("exp(", round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == size_decided_lat - 1], 2), ")")`` which is ca. `r if (!params$EVAL) character() else paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == size_decided_lat - 1], 2)), 6), "%")` whereas size `r if (!params$EVAL) character() else size_decided_lat` leads to a GMPD of ca. `r if (!params$EVAL) character() else paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == size_decided_lat], 2)), 4), "%")`. This is a considerable improvement from size `r if (!params$EVAL) character() else size_decided_lat - 1` to size `r if (!params$EVAL) character() else size_decided_lat`, so another justification for size `r if (!params$EVAL) character() else size_decided_lat`. (Size `r if (!params$EVAL) character() else size_decided_lat + 1` would have resulted in a GMPD of ca. `r if (!params$EVAL) character() else paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == size_decided_lat + 1], 2)), 4), "%")` which is only a small improvement compared to size `r if (!params$EVAL) character() else size_decided_lat`.)

In the predictor ranking up to the selected size of `r if (!params$EVAL) character() else size_decided_lat`, we can see that apart from the noise term `xn.6`, **projpred** has correctly selected the truly relevant predictors first and only then the noise predictors. We can see this more clearly using the following code:
...
```{r plot_vsel_nebin, fig.width = 6, out.width = "75%"}
( gg_nebin <- plot(vs_nebin, stats = "mlpd", deltas = TRUE) )
```

Again, a zoomed plot is more helpful:
```{r plot_vsel_nebin_zoom, fig.width = 6, out.width = "75%"}
gg_nebin + ggplot2::coord_cartesian(ylim = c(-2.5, 0.25))
```

Although the submodels' MLPDs approach the reference model's already from a submodel size of 9 on (taking into account the uncertainty indicated by the error bars), the curve levels off only from a submodel size of 11 on. For a more informed decision, the GMPD could be taken into account again, but for the sake of brevity, we will simply decide for a submodel size of 11 here:
```{r size_man_nebin}
...
We can also inspect further results from the `varsel()` run and, in particular, the predictor ranking:
```{r smmry_vsel_nebin}
smmry_nebin <- summary(
  vs_nebin,
  stats = "mlpd",
...
print(smmry_nebin, digits = 3)
```

As we can see from the predictor ranking, our selected `r if (!params$EVAL) character() else size_decided_nebin` predictor terms lack one truly relevant predictor (`x.9`) and include one noise term (`xn.29`). More explicitly, our selected predictor terms are:
