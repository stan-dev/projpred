...
We now run a variable selection with `latent = TRUE` to request the latent projection (since we have a hold-out test dataset available, we can use `varsel()`'s argument `d_test` instead of `cv_varsel()`):
