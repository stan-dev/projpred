...
In this example, we will illustrate that in case of an exponential-family response distribution (here the Poisson distribution), the latent projection can improve runtime and results of the variable selection compared to **projpred**'s traditional projection.
...
# Piironen and Vehtari (2017, DOI: 10.1214/17-EJS1337SI) (in the data-generating
# code above, we are using an intercept of 2.1; suppose we knew roughly about
# it):
mu_prior <- exp(2)
...
```
On absolute scale (column `mlpd`), we see that submodel size 11 leads to an MLPD of ``r round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == 11], 3)``, i.e., a geometric mean predictive probability (GMPP) of ``r paste0("exp(", round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == 11], 3), ")")`` which is ca. `r paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == 11], 3)), 3), "%")` whereas size 12 leads to a GMPP of ca. `r paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == 12], 3)), 3), "%")`. This is a considerable improvement from size 11 to size 12, so another justification for size 12. (Size 13 would have resulted in a GMPP of ca. `r paste(100 * round(exp(round(smmry_lat$selection[["mlpd"]][smmry_lat$selection$size == 13], 3)), 3), "%")` which is no improvement compared to size 12.)

In the solution path up to the selected size of 12, we can see that one noise term (`xn.6`) has slipped among the selected predictor terms. We can see this more explicitly using the following code:
```{r}
soltrms_lat <- solution_terms(cvvs_lat)
( soltrms_lat_final <- head(soltrms_lat, modsize_decided_lat) )
```
In this case, this suboptimal solution path is due to the L1 search. A forward search would have correctly selected the truly relevant predictors first and only then the noise predictors. (A forward search would also have given a slightly smoother submodel size selection plot.) However, for speeding up the building of this vignette, we use L1 search here.
...
In case of the traditional projection, a forward search would have given markedly better results (in particular, the submodel size selection curve would have been much smoother and all of the noise terms would have been selected after the truly relevant ones). However, the submodel size selection plot would still have been more unstable than in case of the latent projection.

In conclusion, this example showed that the latent projection can be advantageous also for exponential-family models by improving the runtime and the stability of the variable selection, eventually also leading to better variable selection results. This holds especially in case of L1 search, but also (even if not that strongly) for forward search.
