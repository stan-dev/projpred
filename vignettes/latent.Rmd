...
We now run a variable selection using `cv_varsel()` with `latent = TRUE` to state that we want to use the latent projection:
```{r, results='hide'}
library(projpred)
cvvs_lat <- cv_varsel(
  refm_fit_poiss,
...
Although the MLPD seems to be very close to the reference model's MLPD from a submodel size of 9 on, a look at 
a zoomed plot reveals that there is still some discrepancy at size 9 and that size 10 would be a better choice (further down below in the `summary()` output, we will see that on absolute---not difference---scale, the discrepancy at size 9 is not negligible):
```{r}
...
We will skip the post-selection inference here, but note that `proj_predict()` has a new argument `respOrig` and that analogous functionality is available in `proj_linpred()` (argument `transform`) and `predict.refmodel()` (argument `type`).

**TODO (latent)**: STOPPED HERE.
