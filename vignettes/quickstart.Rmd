...
This section shows an example of the variable selection for a logistic regression model (binary classification). Everything is very similar to the Gaussian case. First load the data:
```{r}
load('../data/binomialExample.RData')
```

We then fit the full model:
```{r}
# fit the full model
n <- dim(x)[1]
D <- dim(x)[2]
