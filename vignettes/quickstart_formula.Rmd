...
fetch_data <- function(data_points=NULL, xnew=NULL) {
...
refmodel <- get_refmodel_poc(fit, d$y, formula,
                             predfun=predfun,
...
The variable selection can then be excecuted with the command `cv_varsel_poc`. This returns an object that contains the relevant information about the variable selection, such as the ordering of the variables. The search heuristic can be specified by the keyword `method`.
...
vs <- cv_varsel_poc(refmodel, method='forward')
```
...
proj <- project_poc(vs, nv = 3, ns = 500)
...
pred <- proj_linpred_poc(vs, xnew=xnew, ynew=df_gaussian$y, nv = 6, integrated = TRUE)
```
...
xnew <- df_gaussian[, colnames(df_gaussian) != "y"]
y1_rep <- proj_predict_poc(vs, xnew=xnew[1,,drop=F], nv=6, seed=7560)
