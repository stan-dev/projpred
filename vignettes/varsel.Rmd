...
__Note: The contents of this vignette are outdated. Check the notebook quickstart.Rmd for information about the package!__
...
can use cross-validation (Piironen and Vehtari, 2016). In addition to more
traditional performance metrics such as mean squared error, the procedure also
produces possibly different search paths, the similarity of which can also
...
fit3 <- stan_glm(y ~ x, data = d_train, family = gaussian(),
                 ...
                 iter = ITER, chains = CHAINS, cores = CORES, seed = 12345)
```
...
variable selection only up to 50 variables. In order to find a sufficient 
submodel size, we also save the cross-validated fits in a separate object.

```{r, crime-varsel, results = "hide"}
library(glmproj)
fit3_v <- cv_varsel(fit = fit3, method = 'L1', cv_method = 'LOO', 
                    ns = 200, nv_max = 50)
```
...
varsel_plot(fit3_v, statistics = c('kl','mse','mlpd'), nv_max = 20)
```
...
kable(
  varsel_statistics(fit3_v, nv_max = 10)[, c('size', 'chosen', 'pctch')],
  align = 'c',
...
proj3 <- proj_linpred(fit3_p, xnew =  d_test$x, ynew = d_test$y)
pred_sub_mean <- lapply(proj3, function(proj) colMeans(proj$pred))
...
fit3_v <- cv_varsel(fit = fit3, method = 'L1', cv_method = 'LOO', ns = 200)
```
...
varsel_plot(fit3_v, statistics = c('kl','pctcorr'), deltas = FALSE)
```
...
proj_sub <- proj_linpred(fit3_p, transform = T,
                         xnew = d_test$x, ynew = d_test$y)
pred_full <- posterior_linpred(fit3, transform = T, newdata = d_test)

acc_sub <- sapply(proj_sub, 
                  function(proj) sum(round(colMeans(proj$pred),0)==d_test$y))
