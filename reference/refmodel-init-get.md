...
explicitly by a call to `get_refmodel()`. All non-"typical" reference
model objects will be called "custom" reference model objects.

Some arguments are for \\K\\-fold cross-validation (\\K\\-fold CV) only;
see [`cv_varsel()`](https://mc-stan.org/projpred/reference/cv_varsel.md)
...
- `proj_predfun`: `proj_predfun(fits, newdata)` where:
  ...
  - `fits` accepts a `list` of length \\S\_{\mathrm{prj}}\\ containing
    ...
    its output element `outdmin` (which in turn is the same as the
    return value of `div_minimizer`, except if
    [`project()`](https://mc-stan.org/projpred/reference/project.md) was
    used with an `object` of class `vsel` based on an L1 search as well
