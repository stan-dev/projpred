...
[`cv_varsel()`](https://mc-stan.org/projpred/reference/cv_varsel.md), it
is possible to pass `latent = TRUE` from such a top-level function down
to
[`extend_family()`](https://mc-stan.org/projpred/reference/extend_family.md)
...
[`project()`](https://mc-stan.org/projpred/reference/project.md) or
...
[`plot.vsel()`](https://mc-stan.org/projpred/reference/plot.vsel.md) to
`"secondary_x"` to make the submodel sizes readable in all of the plots
in this vignette.
...
[`extend_family()`](https://mc-stan.org/projpred/reference/extend_family.md)[⁷](#fn7):
...
and the downstream **projpred** steps:
...
```
    ...
    Warning: Some (3 / 500) Pareto k's for the reference model's PSIS-LOO weights
    are > 0.7.
