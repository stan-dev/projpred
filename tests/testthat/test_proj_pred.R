...
if (require(brms)) {
  ...
  vs_list <- lapply(fit_list, varsel_poc, nv_max = nv, verbose = FALSE)
  proj_vind_list <- lapply(vs_list, project_poc, vind = c(2,3), seed = seed)
  proj_all_list <- lapply(vs_list, project_poc, seed = seed, nv=0:nv)


  test_that("proj_linpred: xnew is specified correctly", {
    expect_error(proj_linpred_poc(proj_vind_list),
                 'argument "xnew" is missing, with no default')
    expect_error(proj_linpred_poc(proj_vind_list, xnew = NULL),
                 'must be a data.frame or a matrix')
    expect_error(proj_linpred_poc(proj_vind_list, xnew = x[, 1]),
                 'must be a data.frame or a matrix')
    expect_error(proj_linpred_poc(proj_vind_list, xnew = x, vind = 1:1000),
                 'number of columns in xnew does not match')
    expect_error(proj_linpred_poc(proj_vind_list, xnew = x[, 1:2]),
  ...
  test_that("output of proj_linpred is sensible with fit-object as input", {
    for(i in 1:length(vs_list)) {
      ...
      pl <- proj_linpred_poc(vs_list[[i]], xnew = x, nv = 0:nv)
  ...
  test_that("proj_linpred: error when varsel has not been performed on the object", {
    expect_error(proj_linpred_poc(1, xnew = x),
                 'is not a variable selection object')
    expect_error(proj_linpred_poc(fit_gauss, xnew = x),
                 'is not a variable selection object')
    expect_error(proj_linpred_poc(c(proj_vind_list, list(x)), xnew = x),
                 'contains objects not created by varsel')
  })

  test_that("proj_linpred: specifying ynew incorrectly produces an error", {
    expect_error(proj_linpred_poc(vs_list[["gauss"]], xnew = x, ynew = x[, 1:3]),
                 'y cannot have more than two columns')
    expect_error(proj_linpred_poc(vs_list[["gauss"]], xnew = x, ynew = factor(ys[[1]])),
                 'cannot be a factor')
    expect_error(proj_linpred_poc(vs_list[["poiss"]], xnew = x, ynew = factor(ys[[3]])),
                 'cannot be a factor')
    expect_error(proj_linpred_poc(vs_list[["binom"]], xnew = x, ynew = ys[[1]]),
                 'y values must be 0 <= y <= 1 for the binomial model')
    expect_error(proj_linpred_poc(vs_list[["binom"]], xnew = x, ynew = factor(ys[[1]])),
  ...
  test_that("proj_linpred: specifying ynew has an expected effect", {
    for(i in 1:length(vs_list)) {
      i_inf <- names(vs_list)[i]
      pl <- proj_linpred_poc(vs_list[[i]], xnew = x, ynew = ys[[i]], weightsnew=weights, nv = 0:nv)
      pl2 <- proj_linpred_poc(vs_list[[i]], xnew = x, weightsnew=weights, nv = 0:nv)
      for(j in 1:length(pl)) {
  ...
  test_that("proj_linpred: specifying ynew as a factor works in a binomial model", {
    ...
    pl <- proj_linpred_poc(vs_list[["binom"]], xnew = x, ynew = yfactor)
  ...
  test_that("proj_linpred: specifying weights has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      ...
      if (proj_vind_list[[i]]$family_kl$family != 'binomial') {
        i_inf <- names(proj_vind_list)[i]
        plw <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, ynew = ys[[i]],
                            weightsnew = weights)
        pl <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, ynew = ys[[i]])
  ...
  test_that("proj_linpred: specifying offset has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      plo <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, ynew = ys[[i]], weightsnew=weights,
                          offsetnew = offset)
      pl <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, ynew = ys[[i]], weightsnew=weights)
  ...
  test_that("proj_linpred: specifying transform has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      plt <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, transform = TRUE)
      plf <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, transform = FALSE)
  ...
  test_that("proj_linpred: specifying integrated has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      plt <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, integrated = TRUE)
      plf <- proj_linpred_poc(proj_vind_list[[i]], xnew = x, integrated = FALSE)
  ...
  test_that("proj_linpred: adding more regularization has an expected effect", {
    ...
    for(i in 1:length(vs_list)) {
      ...
      for (j in 1:length(regul)) {
        pred <- proj_linpred_poc(vs_list[[i]], xnew = x, nv = 2, transform = FALSE,
  ...
  test_that("proj_linpred: arguments passed to project work accordingly", {
    for(i in 1:length(vs_list)) {
      ...
      prl1 <- proj_linpred_poc(pr, xnew = x)
      prl2 <- proj_linpred_poc(vs_list[[i]], xnew = x, nv = c(2, 4), nc = 2, ns = 20,
                           intercept = FALSE, regul = 1e-8, seed = 12)
  ...
  test_that("proj_linpred: providing xnew as a data frame works as expected", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      pl <- proj_predict_poc(proj_vind_list[[i]],
    ...
    p1 <- proj_linpred_poc(vs_form, xnew = mtcars, nv = 3, seed = 2)
    p2 <- proj_linpred_poc(vs_form, xnew = get_x(fit_form)[,-1], nv = 3, seed = 2)
    expect_equal(p1, p2)
  })
  ...
  test_that("proj_predict: xnew is specified correctly", {
    expect_error(proj_predict_poc(proj_vind_list),
                 'argument "xnew" is missing, with no default')
    expect_error(proj_predict_poc(proj_vind_list, xnew = NULL),
                 'must be a data.frame or a matrix')
    expect_error(proj_predict_poc(proj_vind_list, xnew = x[, 1]),
                 'must be a data.frame or a matrix')
    expect_error(proj_predict_poc(proj_vind_list, xnew = x, vind = 1:1000),
                 'number of columns in xnew does not match')
    expect_error(proj_predict_poc(proj_vind_list, xnew = x[, 1:2]),
                 'xnew has 2 columns, but vind expects 3 columns')
  })
  ...
  test_that("output of proj_predict is sensible with fit-object as input", {
    for(i in 1:length(vs_list)) {
      i_inf <- names(vs_list)[i]
      pl <- proj_predict_poc(vs_list[[i]], xnew = x, nv = 0:nv)
      expect_length(pl, nv + 1)
      for(j in 1:length(pl))
        expect_equal(ncol(pl[[j]]), n, info = i_inf)
    }
  ...
  test_that("output of proj_predict is sensible with project-object as input", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      pl <- proj_predict_poc(proj_vind_list[[i]], xnew = x)
      expect_equal(ncol(pl), n, info = i_inf)
    }
    for(i in 1:length(proj_all_list)) {
      i_inf <- names(proj_all_list)[i]
      pl <- proj_predict_poc(proj_all_list[[i]], xnew = x)
      expect_length(pl, nv + 1)
      for(j in 1:length(pl))
        expect_equal(ncol(pl[[j]]), n, info = i_inf)
    }
  ...
  test_that("proj_predict: error when varsel has not been performed on the object", {
    expect_error(proj_predict_poc(1, xnew = x),
                 'is not a variable selection object')
    expect_error(proj_predict_poc(fit_gauss, xnew = x),
                 'is not a variable selection object')
    expect_error(proj_predict_poc(c(proj_vind_list, list(x)), xnew = x),
                 'contains objects not created by varsel')
  })

  test_that("proj_predict: specifying ynew has an expected effect", {
    for (i in 1:length(vs_list)) {
      pl <- proj_predict_poc(vs_list[[i]], xnew = x, ynew = ys[[i]], nv = 0:3)
      pl2 <- proj_predict_poc(vs_list[[i]], xnew = x, nv = 0:3)
      for (j in 1:length(pl)) {
  ...
  test_that("proj_predict: specifying ynew as a factor works in a binomial model", {
    yfactor <- factor(rbinom(n, 1, 0.5))
    pl <- proj_predict_poc(vs_list[["binom"]], xnew = x, ynew = yfactor)
    expect_equal(ncol(pl), n)
    expect_true(all(pl %in% c(0, 1)))
  })

  test_that("proj_predict: specifying weightsnew has an expected effect", {
    pl <- proj_predict_poc(proj_vind_list[['binom']], xnew = x, seed = seed)
    plw <- proj_predict_poc(proj_vind_list[['binom']], xnew = x, seed = seed,
                        weightsnew = weights)
  ...
  test_that("proj_predict: specifying offsetnew has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      pl <- proj_predict_poc(proj_vind_list[[i]], xnew = x, draws = iter,
                         seed = seed)
      plo <- proj_predict_poc(proj_vind_list[[i]], xnew = x, draws = iter,
  ...
  test_that("proj_predict: specifying seed_sam has an expected effect", {
    for(i in 1:length(proj_vind_list)) {
      i_inf <- names(proj_vind_list)[i]
      pl1 <- proj_predict_poc(proj_vind_list[[i]], xnew = x, seed = seed)
      pl2 <- proj_predict_poc(proj_vind_list[[i]], xnew = x, seed = seed)
  ...
  test_that("proj_predict: arguments passed to project work accordingly", {
    for(i in 1:length(vs_list)) {
      ...
      pr1 <- project(vs_list[[i]], nv = c(2, 4), nc = 2, ns = 20,
                     intercept = FALSE, regul = 1e-8, seed = 12)
      prp1 <- proj_predict_poc(pr1, xnew = x, draws = 100, seed = 11)
      prp2 <- proj_predict_poc(vs_list[[i]], xnew = x, draws = 100, seed = 11,
