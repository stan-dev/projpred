...
if (require(rstanarm) && require(brms)) {
  ...
  ref_gauss <- init_refmodel(NULL, df_gauss, df_gauss$y, formula, family = f_gauss)
  ref_binom <- init_refmodel(NULL, df_binom, df_binom_$y, formula, family = f_binom)
  ...
  test_that("varsel: length of the penalty vector is checked", {
    vsf <- function(obj, penalty) varsel(obj, method = 'L1', nv_max = nv + 1,
  ...
  test_that('object returned by cv_varsel contains the relevant fields', {
    for(i in 1:length(cvs_list)) {
      ...
      for(j in 1:length(cvs_list[[i]])) {
        ...
        expect_named(cvs_list[[i]][[j]]$summaries$sub[[1]], c('lppd', 'mu', 'w'),
                     info = paste(i_inf, j_inf))
