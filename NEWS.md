...
* Due to internal refactoring, `project()`'s output elements `submodl` and `weights` have been renamed to `outdmin` and `wdraws_prj`, respectively. In general, these elements don't need to be accessed manually, so this should not be a user-facing change.
* Due to internal refactoring, `varsel()`'s and `cv_varsel()`'s output element `d_test` has been replaced with new output elements `type_test` and `y_wobs_test`. In general, these elements don't need to be accessed manually, so this should not be a user-facing change.
