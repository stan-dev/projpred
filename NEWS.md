...
* Introduction of the latent projection [(Catalina et al., 2021)](https://arxiv.org/abs/2109.04702) which is available for the `binomial()`, the `brms::bernoulli()`, and the `brms::cumulative()` response families. Note that the latent projection is currently considered as an experimental feature since it has not been tested thoroughly yet and is also subject to some more theoretical investigations. For the latent projection based on a **brms** reference model, **brms** version [**TODO (latent)**: X.X.X] or later is needed. (GitHub: [**TODO (latent)**: insert PR number])
