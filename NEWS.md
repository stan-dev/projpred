...
* The changelog for version 2.6.0 did not contain a notification that `cvfolds()` had been deprecated and that the new name `cv_folds()` should be used instead. This has been added now (see below), but is repeated here for the sake of clarity. A deprecation warning was already added in version 2.6.0 and will be kept until `cvfolds()` is eventually removed in a future release. (GitHub: #411)
