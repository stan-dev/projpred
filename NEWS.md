...
* Fix a (long-standing) bug causing offsets not to be taken into account appropriately when calculating `<vsel_object>$summaries$ref$mu` in case of `cv_varsel()` with `cv_method = "LOO"`. (GitHub: #186 (partly), #356)
